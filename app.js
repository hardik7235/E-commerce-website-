...
async function fetchAndDisplayProducts() {
    ...
    try {
        ...
        // Clear the 'Loading...' text once the data is received
        container.innerHTML = '';

        // 4. forEach loop: This code will run for every single product
        products.forEach(product => {
            // Hum yahan image_url aur description dono bhejenge taki cart me dikh sake
            const cardHTML = `
                <a href="product-details.html?id=${product.id}&source=products" class="product-card" style="text-decoration: none; color: inherit; cursor: pointer;">
                    <div class="img-container">
                        <img src="${product.image_url}" alt="${product.name}" class="product-img" onerror="this.src='placeholder.jpg'">
                    </div>
                    <div class="product-info">
                        <h3 class="product-title">${product.name}</h3>
                        <span class="product-price">₹ ${product.price}</span>
                        
                       <button class="add-to-cart-btn" onclick="event.preventDefault(); addToCart({
                        id: ${product.id}, 
        ...
        container.innerHTML = "<p>Failed to load products. Please check your connection.</p>";
    }
}
